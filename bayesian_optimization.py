...
class BayesianOptimization(object):
	...
	def __init__(self, f, pbounds, random_state=None, verbose=1, source_observations=[], source_outs=[]):
		"""
		:param f:
	...
	def maximize(self,
		...
		if not self.initialized:
			...
			# x_init = np.vstack(self.source_observations)
			# y_init = np.hstack(self.source_outs)
			# for x, y in zip(x_init, y_init):
			# 	self.space.add_observation(x, y)
            #
			# self.initialized = True
		...
		alpha = self.alpha_0
		beta = self.beta_0 + np.sum(np.square(np.diff([self.y_s, self.y_hat], axis=0))) / 2.
		...
		self.gp.kernel = self.gp.kernel + source_noise
		...
		for i in range(n_iter):
			...
			# self.plog.print_step(source_noise)
			self.gp.fit(self.space.X, self.space.Y)
